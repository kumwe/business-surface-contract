...
final class FieldPresentationSecurityTest extends TestCase
{
    ...
    /** @since 0.2.0 */
    public function testCanonicalProfileNodeCeilingIsExplicitAndExact(): void
    {
        // Root + 194 rows of 512 leaves + final row of 476 leaves = 100000 nodes.
        $input = array_fill(0, 194, array_fill(0, 512, 0));
        $input[] = array_fill(0, 476, 0);
        $model = $this->model(widget: FieldWidget::Text, inputValue: $input, editable: true);
        self::assertSame($input, $model->inputValue);
        $input[194][] = 0;
        $error = $this->assertThrows(
            fn () => $this->model(widget: FieldWidget::Text, inputValue: $input, editable: true),
            InvalidArgumentException::class,
            'The generic canonical 100000-node ceiling is enforced before encoder invocation.',
        );
        self::assertStringContainsString('100000 nodes', $error->getMessage());
    }
    ...
    private function model(
        ...
        return new FieldPresentationModel(
            ...
            $editable,
            false,
            new \Kumwe\BusinessSurface\Contract\Tests\FixtureEncoder(),
            $errors,
            $options,
            $attributes,
            $provenance,
